...
// We want to change handleDownload to accept a scale parameter.
const oldDownloadCode = `  const handleDownload = async () => {
...
const newDownloadCode = `  const handleDownloadClick = () => {
    if (!selectedTemplate) {
      alert("Please select a template first!");
      return;
    }
...
code = code.replace(oldDownloadCode, newDownloadCode);

// 3. Update the button onClick
// Find the download button: <button onClick={handleDownload} disabled={isDownloading} 
code = code.replace(
...
// Insert the modals right before the final </div> which is at the end of the file.
// Because we know the file ends with:
//         </main>
//       </div>
//     </div>
//   );
...
// We can find `</main>` and inject before it.
